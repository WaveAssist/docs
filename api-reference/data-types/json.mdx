...
Learn how to work with JSON (dictionary and list) data in WaveAssist.
...
```python
try:
    # Attempting to store non-JSON-serializable data
    class CustomClass:
        pass
    
    invalid_data = {"obj": CustomClass()}
    waveassist.store_data("invalid", invalid_data)
except waveassist.SerializationError:
